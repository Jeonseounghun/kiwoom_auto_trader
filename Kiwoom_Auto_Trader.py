...
class Mainwindow(QMainWindow):
        ...
        if os.path.exists("./data/portfolio.txt"):
            ...
            for line in lines:
                if line != "":
                    ...
                    self.portfolio_stock_dict.update({scode:{"종목명":code_name,  "현재가":stock_price}})
            f.close()
    ...
    def detail_account_info1(self, sPrevNext="0"):
        ...
        self.kiwoom.dynamicCall("CommRqData(QString, QString, int, QString)", "예수금상세현황요청", "opw00001", sPrevNext,  self.my_screen_info)
    ...
    def detail_account_info2(self, sPrevNext="0"):
        ...
        self.kiwoom.dynamicCall("CommRqData(QString, QString, int, QString)", "계좌평가잔고내역요청", "opw00018", sPrevNext, self.my_screen_info)
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        ...
        if sRQName == "예수금상세현황요청":
            ##### 예수금 데이터 반환
            self.deposit = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, 0, "예수금")
        ...
        ##### 매입금액 등 현황 수신
            ...
            total_eval_profit_loss = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode,
                                                             sRQName, 0, "총평가손익금액")
            total_earning_rate = str((float(self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode,
                                                         sRQName, 0, "총수익률(%)")))/100)
            ...
            for i in range(rows):
                ...
                code = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i, "종목번호")
                code = code.strip()[1:]
                print(code)

                self.account_stock_dict.update({code: {}})
                
                code_name = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i, "종목명")
                stock_quantity = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i,
                                                    "보유수량")
                buy_price = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i, "매입가")
                learn_rate = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i,
                                                "수익률(%)")
                current_price = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i,
                                                    "현재가")
                total_chegual_price = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName,
                                                        i, "매입금액")
                possible_quantity = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i,
                                                        "매매가능수량")
                eval_profit_loss = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i,
                                                        "평가손익")
                ...
                self.item_output.append(
                    [code_name, str(stock_quantity), str(buy_price), str(current_price), str(eval_profit_loss), str(learn_rate)])
            ###############################################################
            ##### 종목을 UI에 띄우기
            ...
            if sPrevNext == "2": #값이 "2"이면 다음 조회항목이 있다는 뜻으로 종목조회 계속
                ...
                self.kiwoom.dynamicCall("SetRealReg(QString,QString,QString,QString)", self.screen_start_stop_num, '',
                ...
                for code in self.portfolio_stock_dict.keys():
                    ...
                    if code:
                        self.kiwoom.dynamicCall("SetRealReg(QString,QString,QString,QString)", screen_num, code, fids, "1")
                        print("스크린번호 배분 완료 %s[코드 : %s, 번호 : %s]" % (cnt, code, screen_num))
    ...
    def realdata_slot(self, sCode, sRealType, sRealData):
        ...
        #### 틱체결이 이루어지면 종목을 호출
            ...
            a = self.kiwoom.dynamicCall("GetCommRealData(QString,int)",sCode, self.realtype.REALTYPE[sRealType]['체결시간'])
            b = self.kiwoom.dynamicCall("GetCommRealData(QString,int)", sCode, self.realtype.REALTYPE[sRealType]['현재가'])
            b = abs(int(b))
            c = self.kiwoom.dynamicCall("GetCommRealData(QString,int)",sCode, self.realtype.REALTYPE[sRealType]['전일대비'])
            c = abs(int(c))
            d = self.kiwoom.dynamicCall("GetCommRealData(QString,int)",sCode, self.realtype.REALTYPE[sRealType]['등락율'])
            d = float(d)
            ...
            #### 매도조건
            if sCode in self.account_stock_dict.keys() and self.account_stock_dict[sCode]['매입가'] > 0:
                ...
                if meme['매매가능수량'] > 0 and (meme_rate > 3.0):
                    ...
                    order_sucess = self.kiwoom.dynamicCall(
                        ...
                        ["매도요청", self.portfolio_stock_dict[sCode]['주문용스크린번호'], self.account_num, 2, sCode,
                         meme['매매가능수량'], 0, self.realtype.SENDTYPE['거래구분']['시장가'], ''])

                    if order_sucess == 0:
                        self.mywidget.text_edit3.append("[매도주문]["+text_time+"][수익률 : %s] %s 매도주문 성공"
                                                        % (meme_rate, self.account_stock_dict[sCode]['종목명']))

                        self.f = open("./data/profit.txt", "a", encoding="utf8")
                        info = "%s\t%s\n" % (self.account_stock_dict[sCode]['종목명'] ,meme_rate)
                        self.f.write(info)
                        self.f.close()
                        ...
                        for line in lines:
                            if line != "":
                                ...
                                if [str(code_name), str(meme_rate)] in self.profit_stock:
                                    pass
                                else:
                                    self.profit_stock.append([str(code_name), str(meme_rate)])
                                    profit_count = len(self.profit_stock)
                                    for j in range(profit_count):
                                        for i, k in enumerate(self.profit_stock[j]):
                                            item = QTableWidgetItem(k)
                                            item.setTextAlignment(Qt.AlignVCenter | Qt.AlignLeft)
                                            self.mywidget.tableWidget_item2.setItem(j, i, item)
                        self.f.close()
                ...
                elif (meme['매매가능수량'] > 0 and meme_rate < -50.0) and sCode not in self.jango_dict.keys():
                    
                    order_sucess = self.kiwoom.dynamicCall(
                        "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                        ["매도요청", self.portfolio_stock_dict[sCode]['주문용스크린번호'], self.account_num, 2, sCode,
                            1, 0, self.realtype.SENDTYPE['거래구분']['시장가'], ''])
             ...
             #### 추가매수 조건 
                ...
                elif (meme_rate <-10  and  (self.account_stock_dict[sCode]['매입가'] * self.account_stock_dict[sCode]['매매가능수량']) < self.buy_money) and sCode not in self.jango_dict.keys() :

                    
                    order_succes = self.kiwoom.dynamicCall(
                        "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                        ["매수요청", self.portfolio_stock_dict[sCode]['주문용스크린번호'], self.account_num, 1, sCode,
                        stock_order, 0, self.realtype.SENDTYPE['거래구분']['시장가'], ''])
                    if order_succes == 0:
                        self.mywidget.text_edit2.append("[매수주문]%s : 추가 매수주문 성공"  % self.account_stock_dict[sCode]['종목명'] + text_time)
            ...
            elif ((d < 0 and sCode in self.portfolio_stock_dict.keys()) and
             (sCode not in self.account_stock_dict.keys() and sCode not in self.jango_dict.keys())):
                stock_order = (self.buy_money * 0.20) // b
                order_succes = self.kiwoom.dynamicCall("SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                                 ["매수요청", self.portfolio_stock_dict[sCode]['주문용스크린번호'], self.account_num, 1, sCode,
                                stock_order, 0, self.realtype.SENDTYPE['거래구분']['시장가'], ''])
                if order_succes == 0:
                    self.mywidget.text_edit2.append("[매수주문]%s : 1차 매수주문 성공" % self.portfolio_stock_dict[sCode]['종목명'] + text_time )
    ...
    def chejan_slot(self, sGubun, nItemCnt, sFIDList):
        if int(sGubun) ==1:
            ...
            sCode = self.kiwoom.dynamicCall("GetChejanData(int)", self.realtype.REALTYPE['잔고']['종목코드'])[1:]
            stock_quan = self.kiwoom.dynamicCall("GetChejanData(int)", self.realtype.REALTYPE['잔고']['보유수량'])
            stock_quan = int(stock_quan)

            buy_price = self.kiwoom.dynamicCall("GetChejanData(int)", self.realtype.REALTYPE['잔고']['매입단가'])
            buy_price = abs(int(buy_price))
            stock_name = self.kiwoom.dynamicCall("GetChejanData(int)", self.realtype.REALTYPE['잔고']['종목명'])
            stock_name = stock_name.strip()
            self.deposit = self.kiwoom.dynamicCall("GetChejanData(int)", self.realtype.REALTYPE['잔고']['예수금'])
            self.deposit = self.change_format(self.deposit)
